...
PIPELINE_SAMPLE_RATE = PIPELINE_FORMAT.sample_rate
SATELLITE_SAMPLE_RATE = SATELLITE_FORMAT.sample_rate
SATELLITE_FRAME_BYTES = SATELLITE_FORMAT.frame_bytes
...
satellite_ws = None
speaker = SatelliteSpeaker(SPEAKER_VOLUME_DEFAULT)
        ...
        try:
            ...
            if response:
                ...
                try:
                    ...
                    speaker_pcm = await asyncio.to_thread(decode_tts, last_tts_audio)
                    await speaker.play(speaker_pcm)
...
async def handle_satellite_ws(request: web.Request):
    ...
    try:
        ...
        await speaker.bind(ws, "speaker" in capabilities)
        ...
        await ws.send_json({"type": "ready", "protocol": 1, "server": "voice-satellite", "volume": speaker.snapshot()["speaker_volume"],
        ...
        async for msg in ws:
            ...
            while len(rx_buffer) >= SATELLITE_FRAME_BYTES:
                ...
                frame = resample_frame(satellite_frame)
...
async def status_handler(request: web.Request):
    payload = dict(status); payload.update(speaker.snapshot()); payload["uptime_s"] = int(time.time() - STARTED); payload["wake_armed"] = time.monotonic() < wake_armed_until
...
async def volume_handler(request: web.Request):
    ...
    return web.json_response({"ok": True, "volume": await speaker.set_volume(volume)})

async def diagnostic_tone_handler(request: web.Request):
    try:
        peak = int(request.match_info["peak"])
        if not 0 <= peak <= 4096: raise ValueError
    except ValueError: raise web.HTTPBadRequest(text="diagnostic peak must be 0..4096")
    pcm = diagnostic_tone(peak); sent = await speaker.play(pcm)
